...
export default function ConnectionsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await invoke<ConnectionRecord[]>("connections:list");
  ...
  async function saveGithub() {
    ...
    setGhMsg(null);
    try {
      const updated = await invoke<ConnectionRecord>("connections:upsert", { id: "github", token: ghToken, org: ghOrg, connected: !!ghToken });
      setGithub(updated);
  ...
  async function saveAtlassian() {
    ...
    setAtMsg(null);
    try {
      const updated = await invoke<ConnectionRecord>("connections:upsert", { id: "atlassian", token: atToken, email: atEmail, org: atSite, connected: !!(atToken && atEmail && atSite) });
