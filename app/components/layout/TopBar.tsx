...
import { DeveloperForm } from "../dashboard/DeveloperForm";
...
export function TopBar({ developers, selectedId, onSelect, onDevelopersChange, title }: TopBarProps) {
  ...
  async function handleAdd(values: { name: string; role: string; team: string; githubUsername: string; atlassianEmail: string }) {
    ...
    if (!res.ok) {
      ...
      throw new Error(error ?? "Failed to create developer");
    }
  ...
  async function handleEdit(values: { name: string; role: string; team: string; githubUsername: string; atlassianEmail: string }) {
    if (!editDev) return;
    const res = await fetch(`/api/developers/${editDev.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(values),
    });
    if (!res.ok) {
      const { error } = await res.json();
      throw new Error(error ?? "Failed to update developer");
    }
    setEditDev(null);
    await onDevelopersChange();
  }
  ...
  async function handleDelete() {
    if (!editDev) return;
    const res = await fetch(`/api/developers/${editDev.id}`, { method: "DELETE" });
    if (!res.ok) {
      const { error } = await res.json();
      throw new Error(error ?? "Failed to delete developer");
    }
    setEditDev(null);
    await onDevelopersChange();
