    ...
    async function load() {
      try {
        const [allRes, assignedRes] = await Promise.all([
          fetch("/api/sources"),
          fetch(`/api/developers/${initial!.id}/sources`),
        ]);
        if (allRes.ok) {
          const data = await allRes.json();
          if (Array.isArray(data)) setAllSources(data);
        }
        if (assignedRes.ok) {
          const data = await assignedRes.json();
          if (Array.isArray(data)) setAssignedIds(new Set(data.map((s: DataSource) => s.id)));
        }
      } catch {
        /* ignore */
      } finally {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (initial && sourcesLoaded) {
        await fetch(`/api/developers/${initial.id}/sources`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sourceIds: [...assignedIds] }),
        });
      }
    } catch (err) {
