...
export function registerDeveloperHandlers() {
  ...
  ipcMain.handle("developers:create", (_e, data: { name: string; role: string; team: string; githubUsername?: string; atlassianEmail?: string }) => {
    if (!data.name?.trim() || !data.role?.trim() || !data.team?.trim()) throw new Error("name, role, and team are required");
    return createDeveloper({ name: data.name.trim(), role: data.role.trim(), team: data.team.trim(), githubUsername: data.githubUsername, atlassianEmail: data.atlassianEmail });
  });

  ipcMain.handle("developers:update", (_e, data: { id: string; name?: string; role?: string; team?: string; githubUsername?: string; atlassianEmail?: string }) => {
    const { id, ...rest } = data;
